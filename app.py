...
# --- Load Default CSV ---
default_df = pd.read_csv("hackathon.csv", encoding="utf-8", engine="python", on_bad_lines="skip")
...
def load_topics_from_df(df: pd.DataFrame) -> List[Dict[str, Any]]:
    ...
    for i, row in df.iterrows():
        ...
        topics.append({
            ...
            "prereqs": prereqs, "difficulty": difficulty,
            "est_hours": est_hours, "resources": resources
        })
    return topics
...
def generate_path(topics, interests, skill_level, hours_per_week,
                  max_seed=8, target_weeks=None, resource_types=None,
                  require_resources=False):
    by_id = {t['id']: t for t in topics}
    interest_set = set(i.lower() for x in interests for i in x.split())

    def score_topic(t):
        tags = set(x.lower() for x in t.get('tags', []))
        tag_score = len(tags & interest_set)
    ...
    needed = set()
    def include_rec(id_):
        if id_ in needed or id_ not in by_id: return
        needed.add(id_)
    ...
    ordered_ids, _ = topo_sort_with_prereqs(selected)
    ordered_topics = [by_id[i] for i in ordered_ids if i in needed]
    ...
    if target_weeks:
        ...
        for t in ordered_topics:
            ...
            while dur > 0:
                ...
                cap = weeks[idx]['hours_left']
                if cap <= 0:
                    idx += 1
                    continue
                use = min(dur, cap)
                weeks[idx]['topics'].append({**t, "scheduled_hours": use})
                weeks[idx]['hours_left'] -= use
                dur -= use
                if weeks[idx]['hours_left'] <= 1e-6:
                    idx += 1
    meta = {"generated_at": datetime.utcnow().isoformat() + "Z"}
    return {"ordered": ordered_topics, "weeks": weeks, "meta": meta}

def start_session_with_url(url, topic_title=None, resource_title=None):
    ...
    st.session_state.time_left = total_seconds or 1800
...
def pause_timer():
    if st.session_state.timer_running:
        ...
        st.session_state.paused_time = st.session_state.time_left - int(elapsed)
    else:
        st.session_state.timer_running = True
        st.session_state.start_time = datetime.utcnow()
...
def reset_timer():
    ...
    total = int(p.get('hours', 0) * 3600 + p.get('minutes', 0) * 60)
    st.session_state.time_left = total or 1800
    st.session_state.start_time = datetime.utcnow()
    st.session_state.timer_running = True
    st.session_state.show_celebration = False
...
with tab_onboard:
    ...
    with st.form("onboard_form", clear_on_submit=False):
        ...
        with col2:
            ...
            duration_amount = st.number_input("Duration Amount", min_value=1, max_value=52, value=4)
        ...
        if submitted:
            if not name:
                ...
                st.session_state.profile = {
                    ...
                    'duration_unit': duration_unit, 'sessions_per_week': repeat_per_week,
                    'goals': goals
                }
...
with tab_paths:
    ...
    resource_types = st.multiselect("🎥 Resource Types", ["video", "article", "interactive", "other"],
                                    default=["video", "article", "interactive"])
    ...
    if st.button("🚀 Generate Learning Path"):
        topics = load_topics_from_df(default_df)
        res = generate_path(topics, interests, skill_level, hours_per_week,
...
with tab_timer:
    ...
    if not profile:
        ...
        if not st.session_state.show_celebration:
            ...
            st.caption("🟢 Running" if st.session_state.timer_running else "⏸️ Paused")
